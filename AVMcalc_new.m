...
for i=1:length(x2)
    for j=1:length(theta2)
        A2(i,j) = -R23x(j)*cos(theta2(j)+pi/2) - R23y(j)*cos(pi/2-theta2(j)+pi/2); % Axial force from mid to pin A (end) of beam
    end
end
...
for i=1:length(x2)
    for j=1:length(theta2)
        V2(i,j) = -R23x(j)*sin(theta2(j)) - R23y(j)*sin(pi/2-theta2(j)); % Axial force from mid to pin A (end) of beam
    end
end
...
for i=1:length(x2)
    for j=1:length(theta2)
        Mom2(i,j) = (-R23x(j)*sin(theta2(j)) - R23y(j)*sin(pi/2-theta2(j)))*x2(i)+M2(j); % Axial force from mid to pin A (end) of beam
    end
end
...
for i=1:length(x4)
    for j=1:length(theta4)
        A4(i,j) = -R64y(j)*cos(theta4(j)-pi/2) - R64x(j)*cos(pi-theta4(j)); % Axial force from mid to pin A (end) of beam
    end
end

figure
surf(X2,THETA2,A4,'edgecolor','none')
